...
if [ "$OPENINGS" == "" ]; then
    OPENINGS="openings/size"$SIZE"-all-1ply"
fi
...
run_tournament() 
{
    ...
    DIRECTORY="jobs/size"$SIZE"-"$NAME1"-vs-"$NAME2
    ...
    ./twogtp.py \
        ...
        --p1cmd "$PROGRAM1 --config $CONFIG1.gtp" --p1name $NAME1 \
        --p2cmd "$PROGRAM2 --config $CONFIG2.gtp" --p2name $NAME2
}
